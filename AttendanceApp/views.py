...
from .models import Head, Teacher, Student, Parent
...
def homepage(request):
    ...
    if request.method == "POST":
        ...
        if choice == 'Head':
            if not Head.objects.filter(email=email).exists():
                email_error = 'Email does not exist'
            else:
                head_object = Head.objects.get(email=email)
                if head_object.password != password:
            ...
            if not Teacher.objects.filter(email=email).exists():
                ...
                teacher_object = Teacher.objects.get(email=email)
                if teacher_object.password != password:
                    password_error = 'Invalid password'
                else:
            ...
            if not Student.objects.filter(email=email).exists():
                ...
                student_object = Student.objects.get(email=email)
                if student_object.password != password:
                    password_error = 'Invalid password'
                else:
            ...
            if not Parent.objects.filter(email=email).exists():
                ...
                parent_object = Parent.objects.get(email=email)
                if parent_object.password != password:
                    password_error = 'Invalid password'
                else:
    ...
    return render(request, 'index.html', {'email_error': email_error, 'password_error': password_error, 'choice_error': choice_error, 'message': message, 'submitted_email': submitted_email, 'submitted_choice': submitted_choice})
            ...
            for student in students:
                ...
                if present_attendance:
                    ...
                    if is_present == True:
                        ...
                        sender = "attendancemanagementsystem2@gmail.com"
                        recipients = [student.parent.email]
                        password = "mmds smzi rbgf ogqb"
...
def head_notice(request):
    notices = Notice.objects.all()

    return render(request, 'head_notice.html', {'notices':notices})
